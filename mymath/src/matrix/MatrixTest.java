...
class MatrixTest {
	...
	void testScaled() {
		double[] numbers = new double[] { 1, 2, 3, 4.5, 6.7, 8.9 };
		Matrix myMatrix = new Matrix(2, 3, numbers);
		assertArrayEquals(new double[6], myMatrix.scaled(0).getElementsByCol());
		assertArrayEquals(numbers, myMatrix.scaled(1).getElementsByRow());
		assertArrayEquals(new double[][] {{2, 4, 6}, {9, 13.4, 17.8}}, myMatrix.scaled(2).getElements());
	}
	...
	void testPlus() {
		...
		Matrix sum = myMatrix1.plus(myMatrix2);
		assertArrayEquals(new double[][] {{2.2, 5.4}, {8.6, 11.5}, {14.7, 17.9}}, sum.getElements());
	}
